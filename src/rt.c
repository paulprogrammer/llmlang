...
// --- Temporal Primitives (libtai-style) ---

#define TAI_OFFSET 4611686018427387904LL // 2^62

long llm_tai_now() {
    // Return current TAI64 label. 
    // Simplified: Use system time + fixed leap second offset (37 as of 2024).
    // In a real libtai, this would read /etc/leapsecs.dat
    return TAI_OFFSET + (long)time(NULL) + 37;
}

// Component indices: 0=Y, 1=M, 2=D, 3=H, 4=m, 5=S
long llm_tai_get(long tai, long component) {
    long s = tai - TAI_OFFSET;
    if (s < 0) s = 0; // Pre-epoch protection

    // TAI is linear, no leap seconds. Every day is 86400s.
    long second = s % 60; s /= 60;
    long minute = s % 60; s /= 60;
    long hour = s % 24; s /= 24;

    // Day to YMD (Standard algorithm)
    long d = s;
