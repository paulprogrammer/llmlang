...
fn main() {
    ...
    if let Expr::Define(name, params, body) = ast1 {
        codegen1.gen_function(&name, params, &body);
        println!("Success!\n");
    }
    ...
    if let Expr::Define(name, params, body) = ast2 {
        codegen2.gen_function(&name, params, &body);
    ...
    if let Expr::Define(name, params, body) = ast3 {
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            codegen3.gen_function(&name, params, &body);
        }));
        if result.is_err() {
