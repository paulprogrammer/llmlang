...
| 20 | **MITIGATED (2026-07-17)** — `llm_join` busy-polls with a 1 ms `timedwait` despite completion signaling | `threads.c:114–144` | wasted CPU while joining | small |
...
| 37 | **PARTIALLY FIXED (2026-07-17)** — Thread-pool has no shutdown path (`pool->shutdown` never set; `cond_signal` not broadcast); lazy `dlopen` in OpenCL dispatch is unsynchronized; OTEL span IDs are clock-derived and collision-prone | `threads.c`, `opencl_dispatch.c:11–22`, `http_server.c:451,458` | resource hygiene / telemetry quality | small–medium |
...
**#25 fixed (2026-07-17, `maturity-work` branch)**: new `codegen/soa.rs` holds the shared SoA helpers — `gen_soa_alloc` (descriptor + column allocation, was pasted in `New`/`Map`/`Filter`), `resolve_soa_field` (field→slot/type resolution, was pasted in `Get`/`Set`; both now share `Get`'s descriptive E007 panic), `gen_soa_col_ptr`, `gen_soa_elem_load`/`store` (8-aligned accesses), and `gen_soa_row_load`/`gen_soa_predicate` (Filter's twice-pasted row-load and predicate-call blocks). The five `expr.rs` arms are now thin compositions of these; generated IR is unchanged apart from value-name labels. Net ~190 lines of duplication removed.
...
**Fourth batch (2026-07-17, `maturity-work` branch)** — findings #20 (mitigated), #37 (partial):
- **#20**: `llm_join`'s `pthread_cond_timedwait` poll interval widened from 1ms to 5ms, cutting the `pool->mutex` lock/unlock churn from repeated `llm_do_work()` calls roughly 5x under heavy nested fork/join. This is a mitigation, not a full fix: the joiner still has to periodically re-check and help drain the pool (removing that entirely risks deadlock if every worker is itself blocked in `llm_join` on a queued child), so some polling remains structurally necessary without a deeper redesign of the wait primitive.
- **#37 (partial)**: the OpenCL dispatch's lazy `dlopen`/`dlsym` init now runs under `pthread_once` (mirroring the `llm_curl_ensure_init` pattern from #18) instead of a racy plain `if (!g_opencl_checked)` check. OTEL span IDs now mix in a global atomic sequence counter alongside the clock reading and thread id, so two spans entered back-to-back on the same thread can no longer collide even where `clock_gettime`'s resolution is coarser than the gap between the two calls. The thread-pool shutdown path (`pool->shutdown` is set but nothing ever sets it, and there's no `llm_shutdown_pool` for callers to invoke) is left open — wiring it in requires deciding what "shutdown" means for in-flight queued work and where a caller would invoke it, which is a design question rather than a mechanical fix.
