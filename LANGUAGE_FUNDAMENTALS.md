...
| **Trap**  | `^` | `^ try_expr fallback_expr` (Recover from panic). |
...
| **Time**  | `tn` / `tns` | `tn` (Now), `tns` (Nano), `tz` (TimeZone). |
...
## 6. Error Handling
*   **Fatal:** Use `! "msg"` (or `` ` "msg" ``) for unrecoverable errors.
